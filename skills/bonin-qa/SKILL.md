...
curl -s -X POST http://localhost:31337/notify \
  ...
  -d '{"message": "Running Extract in bonin-qa"}' \
  > /dev/null 2>&1 &
```
...
```
User: /bonin-qa https://example.com/article
→ WebFetch 获取
→ 找观点骨架 → 设计 Q 链 → 写 A 三段
...
```
User: /bonin-qa ~/Downloads/paper.pdf
