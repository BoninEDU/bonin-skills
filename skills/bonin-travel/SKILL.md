...
**强制 NATIVE 模式。** 本 workflow 是多 skill 管道（Research → ContentAnalysis → bonin-card），不走 Algorithm 七步流程。
...
调用 Skill tool 执行 `bonin-card -i`，输入内容为：城市历史分层 + 核心博物馆清单 + 必看古建清单的精华摘要。一张图看懂这座城市的文明骨架。

**卡片 B — 参观路线速查（长图）：**

调用 Skill tool 执行 `bonin-card -l`，输入内容为：参观路线建议 + 每个地点的核心看点。手机上随时查看。
