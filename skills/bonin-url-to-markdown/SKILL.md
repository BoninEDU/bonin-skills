---
name: bonin-url-to-markdown
description: Fetch any URL and convert to markdown using bonin-fetch CLI (Chrome CDP with site-specific adapters). Built-in adapters for X/Twitter, YouTube transcripts, Hacker News threads, and generic pages via Defuddle. Handles login/CAPTCHA via interaction wait modes. Use when user wants to save a webpage as markdown.
version: 1.61.0
metadata:
  openclaw:
    homepage: https://github.com/BoninEDU/bonin-skills#bonin-url-to-markdown
    requires:
      anyBins:
        - bun
---
...
Fetches any URL via `bonin-fetch` CLI (Chrome CDP + site-specific adapters) and converts it to clean markdown.
...
4. `${READER}` = `{baseDir}/scripts/bonin-fetch`
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `.bonin-skills/bonin-url-to-markdown/EXTEND.md` | Project |
| 2 | `${XDG_CONFIG_HOME:-$HOME/.config}/bonin-skills/bonin-url-to-markdown/EXTEND.md` | XDG |
| 3 | `$HOME/.bonin-skills/bonin-url-to-markdown/EXTEND.md` | User home |
...
- **Q3 — Save** (header "Save"): "Where to save preferences?"
  - "User (Recommended)" — `~/.bonin-skills/` (all projects)
  - "Project" — `.bonin-skills/` (this project only)
...
| `--chrome-profile-dir <path>` | Chrome user data directory (default: `bonin_CHROME_PROFILE_DIR` env or `./bonin-skills/chrome-profile`) |
...
The agent must construct the output file path — `bonin-fetch` does not auto-generate paths.
...
|----------|-------------|
| `bonin_CHROME_PROFILE_DIR` | Chrome user data directory (can also use `--chrome-profile-dir`) |
