...
async function checkApiCredentials(): Promise<void> {
  ...
  const projectEnv = path.join(cwd, '.bonin-skills', '.env');
  const userEnv = path.join(os.homedir(), '.bonin-skills', '.env');
...
async function main(): Promise<void> {
  console.log('=== bonin-post-to-wechat: Permission & Environment Check ===\n');
