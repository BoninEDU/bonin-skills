...
3. `.bonin-skills/.env` with the prefixed key `WECHAT_{ALIAS}_APP_ID`
4. `~/.bonin-skills/.env` with the prefixed key
...
```bash
# Account: bonin
WECHAT_BONIN_APP_ID=your_wechat_app_id
WECHAT_BONIN_APP_SECRET=your_wechat_app_secret
...
${BUN_X} {baseDir}/scripts/wechat-article.ts --markdown <file> --theme default --account bonin
${BUN_X} {baseDir}/scripts/wechat-browser.ts --markdown <file> --images ./photos/ --account bonin
```
...
${BUN_X} {baseDir}/scripts/wechat-api.ts <file> --theme default --account bonin --remote --remote-host other-server.example.com
```
