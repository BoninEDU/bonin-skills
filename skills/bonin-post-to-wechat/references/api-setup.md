...
2. `<cwd>/.bonin-skills/.env` with `WECHAT_APP_ID=...`
3. `$HOME/.bonin-skills/.env` with `WECHAT_APP_ID=...`
...
Where to save?
A) Project-level: .bonin-skills/.env (this project only)
B) User-level: ~/.bonin-skills/.env (all projects)
```
