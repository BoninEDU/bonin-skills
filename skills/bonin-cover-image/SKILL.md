...
metadata:
  openclaw:
    homepage: https://github.com/BoninEDU/bonin-skills#bonin-cover-image
---
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `.bonin-skills/bonin-cover-image/EXTEND.md` | Project |
| 2 | `${XDG_CONFIG_HOME:-$HOME/.config}/bonin-skills/bonin-cover-image/EXTEND.md` | XDG |
| 3 | `$HOME/.bonin-skills/bonin-cover-image/EXTEND.md` | User home |
...
5. **Generate**: Call the chosen backend with the prompt file, output path, aspect ratio.
   - **`codex-imagegen`**: see [references/codex-imagegen.md](references/codex-imagegen.md) for the invocation contract (preferred `bonin-image-gen --provider codex-cli` path, runtime wrapper discovery, parameter notes, stdout schema, batch semantics).
   - **Codex `imagegen` (native)** or other runtime-native tools / `bonin-image-gen` skill: per the rule in `## Image Generation Tools` above.
...
- **Reconfigure interactively** — delete EXTEND.md (or ask "reconfigure bonin-cover-image preferences" / "重新配置"). The next run re-triggers first-time setup.
