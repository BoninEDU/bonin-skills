...
metadata:
  openclaw:
    homepage: https://github.com/BoninEDU/bonin-skills#bonin-image-gen
    requires:
      anyBins:
        - bun
        - npx
...
| Path | Scope |
|------|-------|
| `.bonin-skills/bonin-image-gen/EXTEND.md` | Project |
| `${XDG_CONFIG_HOME:-$HOME/.config}/bonin-skills/bonin-image-gen/EXTEND.md` | XDG |
| `$HOME/.bonin-skills/bonin-image-gen/EXTEND.md` | User home |
...
Legacy compatibility: if `.bonin-skills/bonin-imagine/EXTEND.md` exists and the new path doesn't, the runtime renames it to `bonin-image-gen`. If both exist, the runtime leaves them alone and uses the new path.
...
# Build a batch file from outline.md + prompts/ (e.g. bonin-article-illustrator output)
...
| `BONIN_IMAGE_GEN_MAX_WORKERS` | Override batch worker cap |
| `BONIN_IMAGE_GEN_<PROVIDER>_CONCURRENCY` | Per-provider concurrency (e.g., `BONIN_IMAGE_GEN_REPLICATE_CONCURRENCY`; for codex-cli use `BONIN_IMAGE_GEN_CODEX_CLI_CONCURRENCY`) |
| `BONIN_IMAGE_GEN_<PROVIDER>_START_INTERVAL_MS` | Per-provider start-gap |
| `BONIN_CODEX_IMAGEGEN_BIN` | Override the codex-imagegen wrapper path for the `codex-cli` provider (default: bundled `scripts/codex-imagegen/main.ts`; accepts `.ts` or legacy `.sh`/binary) |
| `BONIN_CODEX_IMAGEGEN_CACHE_DIR` | Enable idempotency cache for the `codex-cli` provider (off by default) |
| `BONIN_CODEX_IMAGEGEN_TIMEOUT_MS` | Per-attempt `codex exec` timeout for the `codex-cli` provider (default: 300000 ms) |
| `BONIN_CODEX_IMAGEGEN_RETRIES` | Wrapper-side retry attempts on retryable errors for the `codex-cli` provider (default: 2) |
| `BONIN_CODEX_IMAGEGEN_LOG_FILE` | Append JSONL diagnostic log for the `codex-cli` provider |

**Load priority**: CLI args > EXTEND.md > env vars > `<cwd>/.bonin-skills/.env` > `~/.bonin-skills/.env`
...
- In non-Codex runtimes with `codex` CLI installed and logged in: use `bonin-image-gen --provider codex-cli` (preferred — it gives you the same retry / cache / batch flow as every other provider). The provider spawns the bundled `scripts/codex-imagegen/main.ts`; the same code lives upstream at `packages/bonin-codex-imagegen/src/main.ts` for standalone callers.
...
| Input is `outline.md` + `prompts/` (e.g. from `bonin-article-illustrator`) | Batch — use `{baseDir}/scripts/build-batch.ts` to assemble the payload | The outline + prompt files already contain everything needed |
...
| `references/codex-oauth-vs-openai-api-key.md` | Why Codex/ChatGPT OAuth image2 entitlement is not usable through bonin-image-gen's standard OpenAI API-key provider |
