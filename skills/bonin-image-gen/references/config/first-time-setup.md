...
description: First-time setup and default model selection flow for bonin-image-gen
---
...
| Choice | Path | Scope |
|--------|------|-------|
| Project | `.bonin-skills/bonin-image-gen/EXTEND.md` | Current project |
| User | `$HOME/.bonin-skills/bonin-image-gen/EXTEND.md` | All projects |
...
- In `bonin-image-gen`, Azure `--model` / `default_model.azure` should be the Azure deployment name, not just the underlying model family.
...
- In `bonin-image-gen`, `quality` is a compatibility preset. It is not a native DashScope parameter.
...
- In `bonin-image-gen`, Z.AI currently exposes text-to-image only; reference images are not wired for this provider.
...
options:
  - label: "google/nano-banana-2 (Recommended)"
    description: "Current default for general Replicate image generation in bonin-image-gen"
