...
- `--quality` is a bonin-imagine preset, not an official DashScope field. The mapping of `normal`/`2k` onto the `qwen-image-2.0*` table is an implementation choice, not an API guarantee
...
DashScope APIs also support `negative_prompt`, `prompt_extend`, and `watermark`, but `bonin-imagine` does not expose them as CLI flags today.
