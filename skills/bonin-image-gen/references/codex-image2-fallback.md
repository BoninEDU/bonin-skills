...
description: Fallback behavior when bonin-image-gen lacks OpenAI API credentials but Codex/native image generation is available
---

# Codex Image2 Fallback

When using `bonin-image-gen` with `--provider openai --model gpt-image-2`, the CLI can fail with:
...
1. Try `bonin-image-gen` when provider credentials are available.
...
3. Prefer a Codex/native raster backend in this order:
   ...
   - `bonin-image-gen --provider codex-cli` (preferred — wraps the bundled `scripts/codex-imagegen/main.ts`; the underlying repo-level package lives at `packages/bonin-codex-imagegen/src/main.ts` for standalone callers), if `codex` CLI is installed/logged in.
...
Avoid implying that `bonin-image-gen --provider openai` can use Codex OAuth without a dedicated provider implementation.
