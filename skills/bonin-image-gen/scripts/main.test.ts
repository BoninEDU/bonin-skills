...
test("parseArgs parses the main bonin-image-gen CLI flags", () => {
...
test("loadExtendConfig renames legacy EXTEND.md when the new path is missing", async () => {
  const root = await makeTempDir("bonin-image-gen-extend-");
  const cwd = path.join(root, "project");
  const home = path.join(root, "home");
  const legacyPath = path.join(cwd, ".bonin-skills", "bonin-imagine", "EXTEND.md");
  const currentPath = path.join(cwd, ".bonin-skills", "bonin-image-gen", "EXTEND.md");
...
test("loadExtendConfig leaves legacy EXTEND.md untouched when both paths exist", async () => {
  const root = await makeTempDir("bonin-image-gen-extend-dual-");
  const cwd = path.join(root, "project");
  const home = path.join(root, "home");
  const legacyPath = path.join(cwd, ".bonin-skills", "bonin-imagine", "EXTEND.md");
  const currentPath = path.join(cwd, ".bonin-skills", "bonin-image-gen", "EXTEND.md");
...
test("batch worker and provider-rate-limit configuration prefer env over EXTEND config", (t) => {
  useEnv(t, {
    BONIN_IMAGE_GEN_MAX_WORKERS: "12",
    BONIN_IMAGE_GEN_GOOGLE_CONCURRENCY: "5",
    BONIN_IMAGE_GEN_GOOGLE_START_INTERVAL_MS: "450",
    BONIN_IMAGE_GEN_ZAI_CONCURRENCY: "4",
  });
...
test("loadBatchTasks and createTaskArgs resolve batch-relative paths", async (t) => {
  const root = await makeTempDir("bonin-image-gen-batch-");
  t.after(() => fs.rm(root, { recursive: true, force: true }));
...
test("path normalization, worker count, and retry classification follow expected rules", () => {
  ...
  assert.equal(
    isRetryableGenerationError(
      new Error("Replicate returned 2 outputs, but bonin-image-gen currently supports saving exactly one image per request."),
    ),
    false,
  );
  assert.equal(
  ...
  assert.equal(
    isRetryableGenerationError(
      new Error("DashScope wan2.7 image models in bonin-image-gen support exactly one output image per request."),
    ),
    false,
  );
  assert.equal(
