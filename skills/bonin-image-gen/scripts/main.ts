...
Environment variables:
  ...
  BONIN_IMAGE_GEN_<PROVIDER>_CONCURRENCY  Override provider concurrency (use underscores: BONIN_IMAGE_GEN_CODEX_CLI_CONCURRENCY)
  BONIN_IMAGE_GEN_<PROVIDER>_START_INTERVAL_MS  Override provider start gap in ms
  BONIN_CODEX_IMAGEGEN_BIN  Path to codex-imagegen wrapper (default: bundled scripts/codex-imagegen/main.ts; accepts .ts or legacy .sh/binary)
  BONIN_CODEX_IMAGEGEN_CACHE_DIR  Enable idempotency cache for codex-cli provider (default: disabled)
...
async function loadEnv(): Promise<void> {
  ...
  const homeEnv = await loadEnvFile(path.join(home, ".bonin-skills", ".env"));
  const cwdEnv = await loadEnvFile(path.join(cwd, ".bonin-skills", ".env"));
...
function getExtendConfigPathPairs(cwd: string, home: string): ExtendConfigPathPair[] {
  return [
    {
      current: path.join(cwd, ".bonin-skills", "bonin-image-gen", "EXTEND.md"),
      legacy: path.join(cwd, ".bonin-skills", "bonin-imagine", "EXTEND.md"),
    },
    {
      current: path.join(home, ".bonin-skills", "bonin-image-gen", "EXTEND.md"),
      legacy: path.join(home, ".bonin-skills", "bonin-imagine", "EXTEND.md"),
    },
  ];
}
...
export function getConfiguredMaxWorkers(extendConfig: Partial<ExtendConfig>): number {
  const envValue = parsePositiveInt(process.env.BONIN_IMAGE_GEN_MAX_WORKERS);
...
export function detectProvider(args: CliArgs): Provider {
  ...
  throw new Error(
    "No API key found. Set GOOGLE_API_KEY, GEMINI_API_KEY, OPENAI_API_KEY, AZURE_OPENAI_API_KEY+AZURE_OPENAI_BASE_URL, OPENROUTER_API_KEY, DASHSCOPE_API_KEY, ZAI_API_KEY, MINIMAX_API_KEY, REPLICATE_API_TOKEN, JIMENG keys, ARK_API_KEY, or AGNES_API_KEY.\n" +
      "Create ~/.bonin-skills/.env or <cwd>/.bonin-skills/.env with your keys."
  );
}
