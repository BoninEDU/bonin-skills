...
async function resolveWrapperPath(): Promise<string> {
  const override = process.env.BONIN_CODEX_IMAGEGEN_BIN;
  if (override) {
    if (!(await exists(override))) {
      throw new Error(
        `Invalid BONIN_CODEX_IMAGEGEN_BIN: ${override} does not exist.`,
      );
    }
    return override;
  }
  ...
  throw new Error(
    `codex-cli wrapper not found at ${BUNDLED_WRAPPER}. ` +
      `Reinstall bonin-image-gen, or set BONIN_CODEX_IMAGEGEN_BIN to a codex-imagegen main.ts (or .sh) path.`,
  );
}
...
export async function generateImage(
  ...
  const sessionDir = path.join(tmpdir(), "bonin-image-gen-codex-cli");
  ...
  const cacheDir = getEnvOverride("BONIN_CODEX_IMAGEGEN_CACHE_DIR");
  if (cacheDir) cliArgs.push("--cache-dir", cacheDir);

  const timeoutMs = parsePositiveInt(process.env.BONIN_CODEX_IMAGEGEN_TIMEOUT_MS);
  if (timeoutMs) cliArgs.push("--timeout", String(timeoutMs));

  const retries = parsePositiveInt(process.env.BONIN_CODEX_IMAGEGEN_RETRIES);
  if (retries !== null) cliArgs.push("--retries", String(retries));

  const logFile = getEnvOverride("BONIN_CODEX_IMAGEGEN_LOG_FILE");
