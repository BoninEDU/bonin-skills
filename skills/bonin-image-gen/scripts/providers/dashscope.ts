...
export async function generateImage(
  ...
  if (spec.family === "wan27" && args.n !== 1) {
    throw new Error(
      "DashScope wan2.7 image models in bonin-image-gen support exactly one output image per request (extra images would be billed but discarded). Remove --n or use --n 1."
    );
  }
