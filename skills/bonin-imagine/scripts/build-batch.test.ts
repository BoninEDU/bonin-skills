...
const scriptPath = path.join(repoRoot, "skills", "bonin-imagine", "scripts", "build-batch.ts");
...
async function makeFixture(): Promise<{
  ...
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "bonin-imagine-build-batch-"));
...
test("build-batch omits default model so bonin-imagine can resolve env or EXTEND defaults", async () => {
