...
test("parseArgs parses the main bonin-imagine CLI flags", () => {
...
test("loadExtendConfig renames legacy EXTEND.md when the new path is missing", async () => {
  const root = await makeTempDir("bonin-imagine-extend-");
  const cwd = path.join(root, "project");
  const home = path.join(root, "home");
  const legacyPath = path.join(cwd, ".bonin-skills", "bonin-image-gen", "EXTEND.md");
  const currentPath = path.join(cwd, ".bonin-skills", "bonin-imagine", "EXTEND.md");
...
test("loadExtendConfig leaves legacy EXTEND.md untouched when both paths exist", async () => {
  const root = await makeTempDir("bonin-imagine-extend-dual-");
  const cwd = path.join(root, "project");
  const home = path.join(root, "home");
  const legacyPath = path.join(cwd, ".bonin-skills", "bonin-image-gen", "EXTEND.md");
  const currentPath = path.join(cwd, ".bonin-skills", "bonin-imagine", "EXTEND.md");
...
test("loadBatchTasks and createTaskArgs resolve batch-relative paths", async (t) => {
  const root = await makeTempDir("bonin-imagine-batch-");
...
test("path normalization, worker count, and retry classification follow expected rules", () => {
  ...
  assert.equal(
    isRetryableGenerationError(
      new Error("Replicate returned 2 outputs, but bonin-imagine currently supports saving exactly one image per request."),
    ),
    false,
  );
  assert.equal(
  ...
  assert.equal(
    isRetryableGenerationError(
      new Error("DashScope wan2.7 image models in bonin-imagine support exactly one output image per request."),
    ),
    false,
  );
  assert.equal(
