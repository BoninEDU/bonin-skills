...
export function validateArgs(_model: string, args: CliArgs): void {
  if (args.referenceImages.length > 0) {
    throw new Error("Z.AI GLM-image currently supports text-to-image only in bonin-imagine. Remove --ref or choose another provider.");
  }

  if (args.n > 1) {
    throw new Error("Z.AI image generation currently returns a single image per request in bonin-imagine.");
  }
}
