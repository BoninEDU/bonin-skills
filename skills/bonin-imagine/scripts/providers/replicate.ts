...
export function validateArgs(model: string, args: CliArgs): void {
  ...
  if (args.imageSize && args.imageSizeSource !== "config") {
    throw new Error("Replicate models in bonin-imagine do not use --imageSize. Use --quality, --ar, or --size instead.");
  }
  ...
  if (args.referenceImages.length > 0 || hasExplicitAspectRatio || args.size) {
    throw new Error(
      `Replicate model ${model} is not in the bonin-imagine compatibility list. Supported families: google/nano-banana*, bytedance/seedream-4.5, bytedance/seedream-5-lite, wan-video/wan-2.7-image, wan-video/wan-2.7-image-pro.`
    );
  }
}
...
export function extractOutputUrl(prediction: PredictionResponse): string {
  ...
  if (Array.isArray(output)) {
    if (output.length !== 1) {
      throw new Error(
        `Replicate returned ${output.length} outputs, but bonin-imagine currently supports saving exactly one image per request.`
      );
    }
