...
Environment variables:
  ...
  bonin_IMAGE_GEN_<PROVIDER>_CONCURRENCY  Override provider concurrency
...
function getExtendConfigPathPairs(cwd: string, home: string): ExtendConfigPathPair[] {
  return [
    {
      current: path.join(cwd, ".bonin-skills", "bonin-imagine", "EXTEND.md"),
      legacy: path.join(cwd, ".bonin-skills", "bonin-image-gen", "EXTEND.md"),
    },
    {
      current: path.join(home, ".bonin-skills", "bonin-imagine", "EXTEND.md"),
      legacy: path.join(home, ".bonin-skills", "bonin-image-gen", "EXTEND.md"),
    },
  ];
}
...
export function getConfiguredProviderRateLimits(
  ...
  for (const provider of ["replicate", "google", "openai", "openrouter", "dashscope", "zai", "minimax", "jimeng", "seedream", "azure"] as Provider[]) {
    const envPrefix = `bonin_IMAGE_GEN_${provider.toUpperCase()}`;
