...
metadata:
  openclaw:
    homepage: https://github.com/BoninEDU/bonin-skills#bonin-imagine
    requires:
      anyBins:
        - bun
        - npx
...
| Path | Scope |
|------|-------|
| `.bonin-skills/bonin-imagine/EXTEND.md` | Project |
| `${XDG_CONFIG_HOME:-$HOME/.config}/bonin-skills/bonin-imagine/EXTEND.md` | XDG |
| `$HOME/.bonin-skills/bonin-imagine/EXTEND.md` | User home |
...
Legacy compatibility: if `.bonin-skills/bonin-image-gen/EXTEND.md` exists and the new path doesn't, the runtime renames it to `bonin-imagine`. If both exist, the runtime leaves them alone and uses the new path.
...
| `bonin_IMAGE_GEN_<PROVIDER>_CONCURRENCY` | Per-provider concurrency (e.g., `bonin_IMAGE_GEN_REPLICATE_CONCURRENCY`) |
...
| Input is `outline.md` + `prompts/` (e.g. from `bonin-article-illustrator`) | Batch — use `scripts/build-batch.ts` to assemble the payload | The outline + prompt files already contain everything needed |
