...
6. 画意向画面 SVG {{SKETCH_SVG}}：精确呈现那幅画面，墨像听调度（含「你」才嵌 bonin-portrait.png；复用 extraction.md 的 defs + 骨架）
...
```bash
node ~/.claude/skills/bonin-card/assets/capture.js \
  /tmp/bonin_library_{name}.html ~/Downloads/{name}.png 1080 1440 fullpage
```

复用 bonin-card 的 capture.js（playwright 已装在 bonin-card/node_modules）。**必须 `fullpage`**——卡片高度自适应内容，不留底部空白。`file://` 引用本地封面 / 头像 / 墨像可直接渲染。
