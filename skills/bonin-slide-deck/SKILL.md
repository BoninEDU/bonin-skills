...
metadata:
  openclaw:
    homepage: https://github.com/BoninEDU/bonin-skills#bonin-slide-deck
    requires:
      anyBins:
        - bun
        - npx
...
At generation time, verify files exist. If `usage: direct` and the backend accepts refs (e.g., `bonin-image-gen --ref`), pass the file on every slide. Otherwise embed extracted `style`/`palette` traits in the prompt text.
...
| Path | Scope |
|------|-------|
| `.bonin-skills/bonin-slide-deck/EXTEND.md` | Project |
| `${XDG_CONFIG_HOME:-$HOME/.config}/bonin-skills/bonin-slide-deck/EXTEND.md` | XDG |
| `$HOME/.bonin-skills/bonin-slide-deck/EXTEND.md` | User home |
...
1. Resolve the image backend via the Image Generation Tools rule at the top — ask once if multiple are installed.
   - **`codex-imagegen` invocation**: when the rule resolves to `codex-imagegen`, see [references/codex-imagegen.md](references/codex-imagegen.md) for the invocation contract (preferred `bonin-image-gen --provider codex-cli` path, runtime wrapper discovery, parameter notes, stdout schema, batch semantics — n=1 per call so slide batches must dispatch one wrapper call per slide).
