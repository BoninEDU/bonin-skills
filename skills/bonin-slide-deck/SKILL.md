...
At generation time, verify files exist. If `usage: direct` and the backend accepts refs (e.g., `bonin-imagine --ref`), pass the file on every slide. Otherwise embed extracted `style`/`palette` traits in the prompt text.
