...
metadata:
  openclaw:
    homepage: https://github.com/BoninEDU/bonin-skills#bonin-infographic
---
...
- If `usage: direct` AND the chosen backend accepts reference images (e.g., `bonin-image-gen` via `--ref`) → pass the file via the backend's ref parameter
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `.bonin-skills/bonin-infographic/EXTEND.md` | Project |
| 2 | `${XDG_CONFIG_HOME:-$HOME/.config}/bonin-skills/bonin-infographic/EXTEND.md` | XDG |
| 3 | `$HOME/.bonin-skills/bonin-infographic/EXTEND.md` | User home |
...
4. Call the chosen backend with the prompt file and output path.
   - **`codex-imagegen` invocation**: when the rule resolves to `codex-imagegen`, see [references/codex-imagegen.md](references/codex-imagegen.md) for the invocation contract (preferred `bonin-image-gen --provider codex-cli` path, runtime wrapper discovery, parameter notes, stdout schema, batch semantics).
...
- **Reconfigure interactively** — delete EXTEND.md (or ask "reconfigure bonin-infographic preferences" / "重新配置"). The next run re-triggers first-time setup.
