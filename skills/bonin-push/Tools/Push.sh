#!/bin/bash
# bonin-push: sync updated bonin-* skills to github repo (bonin-skills),
...
#   bash Push.sh --force     # skip detect, sync all bonin-* skills

set -euo pipefail

# === Configuration (HARDCODED) ===
SKILLS_REPO="$HOME/code/bonin-skills"
...
setup_repo() {
  ...
  if [[ "$actual" != *"BoninEDU/bonin-skills"* ]]; then
    err "$SKILLS_REPO exists but origin is '$actual', not bonin-skills."
...
detect_updates() {
  for local_skill in "$SKILLS_LOCAL"/bonin-*; do
...
# List all bonin-* skills in local (force mode).
list_all_local() {
  for local_skill in "$SKILLS_LOCAL"/bonin-*; do
    [ -d "$local_skill" ] || continue
    local name
...
push_branch() {
  ...
  git commit -m "${prefix}: sync bonin-* skills [$skill_list] (v$new_ver)" --quiet
...
check_readme() {
  ...
  # Extract every bonin-xxx mention from README, dedupe
  readme_skills=$(grep -oE 'bonin-[a-z][a-z0-9-]*' "$readme_master" | sort -u)
  # Skills present locally but absent in README
  missing=$(comm -23 <(echo "$local_skills") <(echo "$readme_skills"))

  if [ -z "$missing" ]; then
    ok "README mentions all local bonin-* skills"
    return 0
  fi
...
if [ "$FORCE" = "1" ]; then
  log "  --force: will sync all local bonin-* skills"
elif [ -z "$UPDATED" ]; then
