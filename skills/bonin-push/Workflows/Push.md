...
一键同步 bonin-* skills 到 github repo（master + md 双分支）。
...
脚本会自动 grep README 里所有 `bonin-xxx` 名字，跟 `~/.claude/skills/bonin-*` 对比。如果有 skill 在 local 但不在 README，*push 直接中止*。
...
| "force", "强推" | `--force` | 跳过 detect，强制 rsync 所有 bonin-* |
| "README 已审" | `--skip-readme-check` | 跳过 README 一致性 gate（其他 check 仍跑）|

## Step 2: 执行脚本

```bash
bash ~/.claude/skills/bonin-push/Tools/Push.sh [--dry-run|--force]
```

脚本逻辑：

1. *Setup*：检查 `$HOME/code/bonin-skills` 是否存在，不存在则 clone
2. *Detect*：对比 `~/.claude/skills/bonin-*` vs `repo/skills/bonin-*`，列出有差异的
3. *Master 推送*：
...
仍需手工 review（自动转换不覆盖的差异）:
  - bonin-xxx/SKILL.md  (正文 `*bold*` 标记——斜体歧义，脚本不动)
