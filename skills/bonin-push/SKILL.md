---
name: bonin-push
description: 把 ~/.claude/skills/bonin-* 里所有更新过的 skills 同步到 github repo (bonin-skills)，先推 master 分支（org-mode 输出风格），再切 md 分支（markdown 输出风格）做基础 markdown 化后推。Use when user says '/bonin-push', 'push skills', '推送 skills', '同步 skills', 'sync bonin', or whenever bonin-* skills get updated and need shipping. NOT FOR pushing non-bonin skills or arbitrary git repos.
user_invocable: true
---

# bonin-push: 推送 bonin-* skills

把本地 `~/.claude/skills/bonin-*` 里改过的 skills，一键同步到 github repo，覆盖 master 和 md 两个分支。

## 仓库路径（硬编码）

```
SKILLS_REPO="$HOME/code/bonin-skills"     # 本地工作 repo
SKILLS_LOCAL="$HOME/.claude/skills"      # 本地 skill 源
REPO_URL="git@github.com:BoninEDU/bonin-skills.git"
```

如果 `$SKILLS_REPO` 不存在，脚本会自动 clone。如果它存在但不是 bonin-skills 的 git repo，脚本会报错退出（不破坏现有目录）。
...
- 列出 `~/.claude/skills/bonin-*` 全部 skill 名
- grep `$SKILLS_REPO/README.md` 里出现的 `bonin-xxx`
...
curl -s -X POST http://localhost:31337/notify \
  ...
  -d '{"message": "Running Push in bonin-push"}' \
  > /dev/null 2>&1 &
```

输出文本：`Running **Push** in **bonin-push**...`
...
```
User: /bonin-push
→ 检测 ~/.claude/skills/bonin-* 中跟 repo 有差异的 skills
...
- *脚本前提是 git credentials 已配好*（ssh key 或 PAT）—— bonin-push 不处理认证，认证失败时直接报错
...
- *如果 md 分支的远端比本地新（Bonin 另一台机器推过）*，脚本会 `pull --rebase` 失败时尝试一次 `reset --hard origin/md` 重新应用——这会丢弃本地未推的 md 分支 commit。脚本前会提示
- *搬迁记录*：repo 历史曾在 `~/.claude.backup-20260502/bonin-skills-repo/`（路径名带 backup 是历史遗留），2026-05-02 搬到 `~/code/bonin-skills/`
