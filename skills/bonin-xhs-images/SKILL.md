---
name: bonin-xhs-images
description: Generates infographic image card series with 12 visual styles, 8 layouts, and 3 color palettes. Breaks content into 1-10 cartoon-style image cards optimized for social media engagement. Use when user mentions "小红书图片", "小红书种草", "小绿书", "微信图文", "微信贴图", "image cards", "图片卡片", bonin-xhs-images, or wants social media infographic series.
version: 2.0.1
metadata:
  openclaw:
    homepage: https://github.com/BoninEDU/bonin-skills#bonin-xhs-images
---
...
| Path | Scope |
|------|-------|
| `.bonin-skills/bonin-xhs-images/EXTEND.md` | Project |
| `${XDG_CONFIG_HOME:-$HOME/.config}/bonin-skills/bonin-xhs-images/EXTEND.md` | XDG |
| `$HOME/.bonin-skills/bonin-xhs-images/EXTEND.md` | User home |
...
**`codex-imagegen` invocation**: when the rule resolves to `codex-imagegen`, see [references/codex-imagegen.md](references/codex-imagegen.md) for the invocation contract (preferred `bonin-image-gen --provider codex-cli` path, runtime wrapper discovery, parameter notes, stdout schema, batch semantics — n=1 per call so card batches must dispatch one wrapper call per card; the wrapper does NOT accept `--sessionId`, so chain consistency must come from `--ref` per Step 3 above).
...
- **Reconfigure interactively** — delete EXTEND.md (or ask "reconfigure bonin-xhs-images preferences" / "重新配置"). The next run re-triggers first-time setup.
