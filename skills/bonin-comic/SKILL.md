...
metadata:
  openclaw:
    homepage: https://github.com/BoninEDU/bonin-skills#bonin-comic
    requires:
      anyBins:
        - bun
        - npx
...
3. **Auto-select** (when the preference is `auto`, unset, or the pinned backend isn't available):
   - **Codex (`imagegen`)** — first, inspect your available-skills / tool inventory. If a skill named `imagegen` is listed, you are running inside Codex and MUST use it: invoke via the `Skill` tool with `skill: "imagegen"`, passing the saved prompt file's content (plus output path and aspect ratio per Codex `imagegen`'s own args). Codex `imagegen` is the official raster backend in that runtime and outranks any non-native skill (e.g., `bonin-image-gen`) unless the user has explicitly pinned a different `preferred_image_backend`.
   - **Codex via `codex exec` (`codex-imagegen`)** — if the current runtime exposes no native `imagegen` skill but the `codex` CLI is on `PATH` with an active `codex login`, route through `bonin-image-gen --provider codex-cli` (preferred), or — if bonin-image-gen is unavailable — invoke the bundled wrapper directly. Details, parameters, and the runtime-discovery procedure live in [references/codex-imagegen.md](references/codex-imagegen.md) — load that file only when this branch is selected.
   ...
   - Otherwise, if exactly one non-native backend is installed (e.g., `bonin-image-gen`), use it.
...
Concrete tool names (`imagegen`, `GenerateImage`, `image_generate`, `bonin-image-gen`) above are examples — substitute the local equivalents under the same rule.
...
**Pick a backend once per session** using the `## Image Generation Tools` rule at the top. If the backend is a repo skill (e.g., `bonin-image-gen`), read its `SKILL.md` and use its documented interface rather than its scripts.

**`codex-imagegen` invocation**: when the rule resolves to `codex-imagegen`, see [references/codex-imagegen.md](references/codex-imagegen.md) for the invocation contract (preferred `bonin-image-gen --provider codex-cli` path, runtime wrapper discovery, parameter notes, stdout schema, batch semantics — n=1 per call so page batches must dispatch one wrapper call per page).
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `.bonin-skills/bonin-comic/EXTEND.md` | Project |
| 2 | `$HOME/.bonin-skills/bonin-comic/EXTEND.md` | User home |
...
EXTEND.md lives at `.bonin-skills/bonin-comic/EXTEND.md` (project) or `~/.bonin-skills/bonin-comic/EXTEND.md` (user). Three ways to change it:

- **Edit directly** — open EXTEND.md and change fields. Full schema: `references/config/preferences-schema.md`.
- **Reconfigure interactively** — delete EXTEND.md (or ask "reconfigure bonin-comic preferences" / "重新配置"). The next run re-triggers first-time setup.
- **Common one-line edits**:
  - `preferred_image_backend: auto` — default; runtime-native tool wins, falls back to the only installed backend, asks only if multiple non-native are present.
  - `preferred_image_backend: codex-imagegen` — pin to Codex's built-in.
  - `preferred_image_backend: bonin-image-gen` — pin to the bonin-image-gen skill.
