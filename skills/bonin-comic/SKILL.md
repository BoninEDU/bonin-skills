...
3. **Auto-select** (when the preference is `auto`, unset, or the pinned backend isn't available):
   ...
   - Otherwise, if exactly one non-native backend is installed (e.g., `bonin-imagine`), use it.
...
Concrete tool names (`imagegen`, `image_generate`, `bonin-imagine`) above are examples — substitute the local equivalents under the same rule.
...
**Pick a backend once per session** using the `## Image Generation Tools` rule at the top. If the backend is a repo skill (e.g., `bonin-imagine`), read its `SKILL.md` and use its documented interface rather than its scripts.
...
- **Common one-line edits**:
  ...
  - `preferred_image_backend: bonin-imagine` — pin to the bonin-imagine skill.
