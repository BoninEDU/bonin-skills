...
metadata:
  openclaw:
    homepage: https://github.com/BoninEDU/bonin-skills#bonin-post-to-weibo
    requires:
      anyBins:
        - bun
        - npx
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `.bonin-skills/bonin-post-to-weibo/EXTEND.md` | Project |
| 2 | `${XDG_CONFIG_HOME:-$HOME/.config}/bonin-skills/bonin-post-to-weibo/EXTEND.md` | XDG |
| 3 | `$HOME/.bonin-skills/bonin-post-to-weibo/EXTEND.md` | User home |
...
If a script fails with `Chrome debug port not ready` or `Unable to connect`, kill only the CDP Chrome instances (those with `--remote-debugging-port` AND the bonin-skills profile), then retry:

```bash
pkill -f "remote-debugging-port.*bonin-skills/chrome-profile" 2>/dev/null; sleep 2
```

**CRITICAL**: Never kill all Chrome processes (`pkill -f "Google Chrome"`). Only kill Chrome instances launched by CDP with the bonin-skills profile directory. The user may have regular Chrome windows open.
