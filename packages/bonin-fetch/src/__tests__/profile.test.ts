...
const originalProfile = process.env.bonin_CHROME_PROFILE_DIR;

afterEach(() => {
  if (originalProfile === undefined) {
    delete process.env.bonin_CHROME_PROFILE_DIR;
  } else {
    process.env.bonin_CHROME_PROFILE_DIR = originalProfile;
  }
});

describe("resolveChromeProfileDir", () => {
  test("uses bonin_CHROME_PROFILE_DIR when set", () => {
    process.env.bonin_CHROME_PROFILE_DIR = "/tmp/bonin-profile";
    expect(resolveChromeProfileDir()).toBe("/tmp/bonin-profile");
  });

  test("falls back to shared bonin-skills profile path", () => {
    delete process.env.bonin_CHROME_PROFILE_DIR;
    const resolved = resolveChromeProfileDir();
    if (process.platform === "darwin") {
      expect(resolved).toBe(path.join(os.homedir(), "Library", "Application Support", "bonin-skills", "chrome-profile"));
    } else if (process.platform === "win32") {
      expect(resolved.endsWith(path.join("bonin-skills", "chrome-profile"))).toBe(true);
    } else {
      expect(resolved.endsWith(path.join("bonin-skills", "chrome-profile"))).toBe(true);
    }
  });
...
describe("ensureChromeProfileDir", () => {
  test("creates the profile directory when missing", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "bonin-fetch-profile-"));
