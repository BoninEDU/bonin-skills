...
`bonin-fetch` is a Bun CLI built on Chrome CDP. Give it a URL and it returns
...
- Chrome profile defaults to `bonin-skills/chrome-profile`
...
```bash
bunx bonin-fetch https://example.com
```

You can also install it globally:

```bash
npm install -g bonin-fetch
```
...
bunx bonin-fetch https://example.com
bonin-fetch https://example.com
bonin-fetch https://example.com --format markdown --output article.md
bonin-fetch https://example.com --format markdown --output article.md --download-media
bonin-fetch https://x.com/jack/status/20 --format json --output article.json
bonin-fetch https://x.com/jack/status/20 --json
bonin-fetch https://x.com/jack/status/20 --wait-for interaction
bonin-fetch https://x.com/jack/status/20 --wait-for force
bonin-fetch https://x.com/jack/status/20 --chrome-profile-dir ~/Library/Application\\ Support/bonin-skills/chrome-profile
```

## Options

```bash
bonin-fetch <url> [options]
...
Options:
  ...
  --chrome-profile-dir <path>
                        Chrome user data dir. Defaults to bonin_CHROME_PROFILE_DIR
                        or bonin-skills/chrome-profile
