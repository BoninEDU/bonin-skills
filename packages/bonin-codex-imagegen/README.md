...
This package implements the `preferred_image_backend: codex-imagegen` config key referenced across the `bonin-skills` plugin and is the engine behind `bonin-image-gen --provider codex-cli`.

## Layout

```
packages/bonin-codex-imagegen/
├── src/
...
```bash
cd packages/bonin-codex-imagegen
bun test
```
