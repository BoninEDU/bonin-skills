...
test("parseConventionalCommitSubject accepts scoped, unscoped, and breaking subjects", () => {
  assert.deepEqual(parseConventionalCommitSubject("fix(bonin-post-to-wechat): repair editor paste"), {
    type: "fix",
    scope: "bonin-post-to-wechat",
    breaking: false,
...
test("changedSkillsForPaths returns sorted unique skills", () => {
  assert.deepEqual(
    changedSkillsForPaths([
      "README.md",
      "skills/bonin-post-to-wechat/scripts/wechat-article.ts",
      "skills/bonin-post-to-wechat/SKILL.md",
      "skills/bonin-url-to-markdown/SKILL.md",
    ]),
    ["bonin-post-to-wechat", "bonin-url-to-markdown"],
  );
});
...
test("validateSkillReleaseCommit rejects non-conventional skill commit subjects", () => {
  const failures = validateSkillReleaseCommit({
    ...
    paths: ["skills/bonin-post-to-wechat/scripts/wechat-article.ts"],
  });

  assert.equal(failures.length, 1);
  assert.match(failures[0]!.message, /Conventional Commit/);
  assert.match(formatSkillReleaseFailures(failures), /fix\(bonin-post-to-wechat\):/);
});
