...
现在这个仓库支持把每个 `skills/bonin-*` 目录作为独立 ClawHub skill 发布。
...
/plugin install bonin-skills@bonin-skills
```
...
| 插件 | 说明 | 包含内容 |
|------|------|----------|
| **bonin-skills** | 提供内容生成、AI 后端和日常效率工具技能 | 仓库中的全部 skills，仍按下方的内容技能、AI 生成技能、工具技能三个分类展示 |
...
```bash
# 自动选择风格和布局
/bonin-xhs-images posts/ai-future/article.md

# 指定风格
/bonin-xhs-images posts/ai-future/article.md --style notion

# 指定布局
/bonin-xhs-images posts/ai-future/article.md --layout dense

# 组合风格和布局
/bonin-xhs-images posts/ai-future/article.md --style notion --layout list

# 覆盖配色
/bonin-xhs-images posts/ai-future/article.md --style notion --palette macaron

# 直接输入内容
/bonin-xhs-images 今日星座运势

# 非交互模式（跳过所有确认，适用于定时任务）
/bonin-xhs-images posts/ai-future/article.md --yes
/bonin-xhs-images posts/ai-future/article.md --yes --preset knowledge-card
```
...
```bash
# 根据内容自动推荐组合
/bonin-infographic path/to/content.md

# 指定布局
/bonin-infographic path/to/content.md --layout pyramid

# 指定风格（默认：craft-handmade）
/bonin-infographic path/to/content.md --style technical-schematic

# 同时指定布局和风格
/bonin-infographic path/to/content.md --layout funnel --style corporate-memphis

# 指定比例（预设名称或自定义 W:H）
/bonin-infographic path/to/content.md --aspect portrait
/bonin-infographic path/to/content.md --aspect 3:4
```
...
```bash
# 主题描述 —— 技能分析并提出方案
/bonin-diagram "JWT 认证流程是怎么工作的"
/bonin-diagram "Kubernetes 架构" --type structural
/bonin-diagram "OAuth 2.0 流程"  --type sequence

# 文件路径 —— 技能读取、分析并提出方案
/bonin-diagram path/to/article.md

# 语言和输出路径
/bonin-diagram "微服务架构" --lang zh
/bonin-diagram "build pipeline" --out docs/build-pipeline.svg
```
...
```bash
# 根据内容自动选择所有维度
/bonin-cover-image path/to/article.md

# 快速模式：跳过确认，使用自动选择
/bonin-cover-image path/to/article.md --quick

# 指定维度（5D 系统）
/bonin-cover-image path/to/article.md --type conceptual --palette cool --rendering digital
/bonin-cover-image path/to/article.md --text title-subtitle --mood bold

# 风格预设（向后兼容的简写方式）
/bonin-cover-image path/to/article.md --style blueprint

# 指定宽高比（默认：16:9）
/bonin-cover-image path/to/article.md --aspect 2.35:1

# 纯视觉（不含标题文字）
/bonin-cover-image path/to/article.md --no-title
```
...
```bash
# 从 markdown 文件生成
/bonin-slide-deck path/to/article.md

# 指定风格和受众
/bonin-slide-deck path/to/article.md --style corporate
/bonin-slide-deck path/to/article.md --audience executives

# 指定页数
/bonin-slide-deck path/to/article.md --slides 15

# 仅生成大纲（不生成图片）
/bonin-slide-deck path/to/article.md --outline-only

# 指定语言
/bonin-slide-deck path/to/article.md --lang zh
```
...
```bash
# 从素材文件生成（自动选择画风 + 基调）
/bonin-comic posts/turing-story/source.md

# 指定画风和基调
/bonin-comic posts/turing-story/source.md --art manga --tone warm
/bonin-comic posts/turing-story/source.md --art ink-brush --tone dramatic

# 使用预设（包含特殊规则）
/bonin-comic posts/turing-story/source.md --style ohmsha
/bonin-comic posts/turing-story/source.md --style wuxia

# 指定布局和比例
/bonin-comic posts/turing-story/source.md --layout cinematic
/bonin-comic posts/turing-story/source.md --aspect 16:9

# 指定语言
/bonin-comic posts/turing-story/source.md --lang zh

# 直接输入内容
/bonin-comic "图灵的故事与计算机科学的诞生"
```
...
```bash
# 根据内容自动选择类型和风格
/bonin-article-illustrator path/to/article.md

# 组合类型和风格
/bonin-article-illustrator path/to/article.md --type flowchart --style notion

# 使用色板覆盖
/bonin-article-illustrator path/to/article.md --style vector-illustration --palette macaron
```
...
# 发布 X 文章
/bonin-post-to-x --article path/to/article.md
```
...
```bash
/bonin-post-to-wechat 贴图 --markdown article.md --images ./photos/
/bonin-post-to-wechat 贴图 --markdown article.md --image img1.png --image img2.png --image img3.png
/bonin-post-to-wechat 贴图 --title "标题" --content "内容" --image img1.png --submit
```

**文章模式** - 完整 markdown/HTML 富文本格式：

```bash
/bonin-post-to-wechat 文章 --markdown article.md
/bonin-post-to-wechat 文章 --markdown article.md --theme grace
/bonin-post-to-wechat 文章 --html article.html
```
...
```bash
# 添加到 .bonin-skills/.env（项目级）或 ~/.bonin-skills/.env（用户级）
...
```bash
mkdir -p .bonin-skills/bonin-post-to-wechat
```

创建 `.bonin-skills/bonin-post-to-wechat/EXTEND.md`：
...
```bash
# 发布文字
/bonin-post-to-weibo "Hello Weibo!"

# 发布带图片
/bonin-post-to-weibo "看看这个" --image photo.png

# 发布带视频
/bonin-post-to-weibo "看这个" --video clip.mp4
```
...
```bash
# 发布文章
/bonin-post-to-weibo --article article.md

# 带封面图
/bonin-post-to-weibo --article article.md --cover cover.jpg
```
...
```bash
/bonin-danger-gemini-web "你好，Gemini"
/bonin-danger-gemini-web --prompt "解释量子计算"
```

**图片生成：**

```bash
/bonin-danger-gemini-web --prompt "一只可爱的猫" --image cat.png
/bonin-danger-gemini-web --promptfiles system.md content.md --image out.png
```
...
```bash
# 默认：带时间戳的 Markdown
/bonin-youtube-transcript https://www.youtube.com/watch?v=VIDEO_ID

# 指定语言（按优先级排列）
/bonin-youtube-transcript https://youtu.be/VIDEO_ID --languages zh,en,ja

# 章节分段 + 说话人识别
/bonin-youtube-transcript https://youtu.be/VIDEO_ID --chapters --speakers

# SRT 字幕格式
/bonin-youtube-transcript https://youtu.be/VIDEO_ID --format srt

# 列出可用字幕
/bonin-youtube-transcript https://youtu.be/VIDEO_ID --list
```
...
```bash
# 自动模式（默认）- 页面加载后立即抓取
/bonin-url-to-markdown https://example.com/article

# 等待模式 - 适用于需要登录的页面
/bonin-url-to-markdown https://example.com/private --wait

# 保存到指定文件
/bonin-url-to-markdown https://example.com/article -o output.md
```
...
```bash
# 将推文转换为 markdown
/bonin-danger-x-to-markdown https://x.com/username/status/123456

# 保存到指定文件
/bonin-danger-x-to-markdown https://x.com/username/status/123456 -o output.md

# JSON 输出
/bonin-danger-x-to-markdown https://x.com/username/status/123456 --json

# 下载媒体文件（图片/视频）到本地
/bonin-danger-x-to-markdown https://x.com/username/status/123456 --download-media
```
...
```bash
/bonin-compress-image path/to/image.png
/bonin-compress-image path/to/images/ --quality 80
```
...
```bash
# 格式化 markdown 文件
/bonin-format-markdown path/to/article.md

# 格式化指定文件
/bonin-format-markdown path/to/draft.md
```
...
```bash
# 基础转换
/bonin-markdown-to-html article.md

# 主题 + 颜色
/bonin-markdown-to-html article.md --theme grace --color red

# 将普通外链转换为文末引用
/bonin-markdown-to-html article.md --cite
```
...
```bash
# 创建用户级配置目录
mkdir -p ~/.bonin-skills

# 创建 .env 文件
cat > ~/.bonin-skills/.env << 'EOF'
# OpenAI
...
```bash
mkdir -p .bonin-skills
# 将 .bonin-skills/.env 添加到 .gitignore 避免提交密钥
echo ".bonin-skills/.env" >> .gitignore
```
...
**扩展路径**（按优先级检查）：
1. `.bonin-skills/<skill-name>/EXTEND.md` - 项目级（团队/项目特定设置）
2. `~/.bonin-skills/<skill-name>/EXTEND.md` - 用户级（个人偏好设置）

**示例**：为 `bonin-cover-image` 自定义品牌配色：

```bash
mkdir -p .bonin-skills/bonin-cover-image
```

然后创建 `.bonin-skills/bonin-cover-image/EXTEND.md`：
...
```bash
HTTP_PROXY=http://127.0.0.1:7890 HTTPS_PROXY=http://127.0.0.1:7890 /bonin-danger-gemini-web "你好"
```
...
[![Star History Chart](https://api.star-history.com/svg?repos=BoninEDU/bonin-skills&type=Date)](https://www.star-history.com/#BoninEDU/bonin-skills&Date)
